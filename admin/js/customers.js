                ...
                if (resp.status == 202) {
                    ...
                    $.each(resp.message, function (index, value) {
                        ...
                        customersHTML += '<tr>' +
                                ...
                                '<td>' + value.address + '</td>' +
                                '<td>' + value.email + '</td>' +
                                '<td><a cus_id="' + value.cus_id + '" class="btn btn-sm btn-danger delete-customer" style="color:#fff;"><i class="fas fa-trash-alt"></i></a></td>' +
                                '</tr>'
    ...
    $(document.body).on('click', '.delete-customer', function () {
        ...
        if (confirm("Are you sure to delete this item ?")) {
            $.ajax({
                url: '../admin/classes/Customers.php',
                method: 'POST',
                data: {DELETE_CUSTOMERS: 1, cus_id: cus_id},
                success: function (response) {
                    console.log(response);
                    var resp = $.parseJSON(response);
                    if (resp.status == 202) {
